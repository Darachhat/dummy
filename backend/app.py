...
def start_payment(account_id: int, reference_number: str, service_id: int,
    ...
    # create intent
    pi = Payment(user_id=user_id, account_id=a.id, reference_number=reference_number,
                 service_id=s.id, session_id=inv["session_id"],
                 amount_cents=inv["amount_cents"], currency=inv["currency"], status="started")
    db.add(pi); db.commit(); db.refresh(pi)
    return {"payment_id": pi.id, "service": s.name, **inv}
...
def confirm_payment(
    ...
    if a.balance_cents < pi.amount_cents:
...
def transactions(account_id: int | None = None, user_id: int = Depends(get_user_id), db: Session = Depends(db_sess)):
    ...
    return [{"id":t.id,"account_id":t.account_id,"reference_number":t.reference_number,
             "amount_cents":t.amount_cents,"currency":t.currency,"direction":t.direction,
             "description":t.description,"created_at":t.created_at.isoformat()} for t in items]
